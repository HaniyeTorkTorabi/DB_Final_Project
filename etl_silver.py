...
def run_silver_etl():
    ...
    df['Time_Category'] = pd.cut(df['Hour'], bins=bins, labels=labels, right=False, include_lowest=True, ordered=False)
    ...
    cols_to_drop = [
        'Date', 'Time', 'Reason_for_cancelling_by_Customer',
    ...
    df.to_sql('cleaned_dataset', engine, schema='silver', if_exists='replace', index=False)
